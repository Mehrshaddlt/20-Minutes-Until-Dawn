...
public class MainMenuView implements Screen {
    ...
    private void createUI() {
        ...
        // Add debugging before trying to load
        System.out.println("======= SCORE LOADING DEBUG =======");
        System.out.println("Current user: " + (user == null ? "null" : user.getUsername()));

        // Get the score from the saved JSON file with better error handling
        System.out.println("======= SCORE LOADING DEBUG =======");
        System.out.println("Current user: " + (user == null ? "null" : user.getUsername()));
        System.out.println("Is guest? " + (user == null ? "N/A" : user.isGuest()));
        System.out.println("User object details: " + (user == null ? "null" : user.toString()));
...
// Get the score from the saved JSON file with better error handling
        ...
        try {
            ...
            if (user != null) {
                ...
                // Force load from file regardless of guest status
                String filePath = "users/" + username + ".json";
                FileHandle userFile = Gdx.files.local(filePath);

                if (userFile.exists()) {
                    System.out.println("User file exists at: " + filePath);
                    String fileContent = userFile.readString();
                    System.out.println("File content: " + fileContent);

                    // Load fresh data
                    User freshUser = SaveManager.loadUser(username);
                    if (freshUser != null) {
                        score = freshUser.getTotalScore();
                        System.out.println("Successfully loaded score: " + score);

                        // Update controller
                        controller.setUser(freshUser);
                    } else {
                        System.out.println("SaveManager returned null for: " + username);
            ...
            System.out.println("ERROR loading score: " + e.getMessage());
            e.printStackTrace();
            if (user != null) {
                score = user.getTotalScore();
            }
